...
// Optional env: RESEND_FROM (e.g., "Contact <contact@yourdomain.com>")
const resend = new Resend(process.env.RESEND_API_KEY);

const RECIPIENT = "advertisement@mapskogroup.com";
...
const formatHtml = (data: FormPayload) => {
  ...
  return `
    <div style="font-family: Arial, sans-serif; color: #111; line-height: 1.6;">
      <h2 style="margin-bottom: 12px;">Contact Form Submission</h2>
...
export async function POST(req: Request) {
  try {
    ...
    const { error } = await resend.emails.send({
      from: fromAddress,
      to: [RECIPIENT],
      subject: `New Contact Form - ${payload.subject || "No Subject"}`,
